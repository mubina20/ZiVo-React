...
export function MobileStories(props: any) {
            ...
            {filteredPosts && filteredPosts.length > 0 ? (
                filteredPosts.map((post: Post) => (
                    post.post_type === "photoStory" ? (
                        // Photo Post
                        <div key={post._id}>
                            <div className="mobile_story_post" onClick={() => handleStorySelct(post?._id, "photoStory")}>
                                <img src={`${serverApi}/${post?.post_content}`} alt="" width="300px"/>
                            </div>
                        </div>
                    ) : post.post_type === "articleStory" ? (
                        // Article Post
                        <div className="mobile_story_post" onClick={() => handleStorySelct(post?._id, "articleStory")}>
                            <div 
                        ...
                        // Video Post
                        <div key={post._id}>
                            <div className="mobile_story_post" onClick={() => handleStorySelct(post?._id, "videoStory")}>
                                <video
                                    loop
                                    // playsInline
                                    // controls
